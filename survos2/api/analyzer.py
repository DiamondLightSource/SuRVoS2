...
@hug.get()
def object_detection_stats(
    workspace: String,
    predicted_objects_id: DataURI,
    gold_objects_id: DataURI,
    dst: DataURI,
) -> "OBJECTS":

    logger.debug(f"Calculating stats on objects: {predicted_objects_id}")

    src = DataModel.g.dataset_uri(
        ntpath.basename(predicted_objects_id), group="objects"
    )
    logger.debug(f"Getting objects {src}")
    with DatasetManager(src, out=None, dtype="float32", fillvalue=0) as DM:
        ds_objects = DM.sources[0]
    entities_fullname = ds_objects.get_metadata("fullname")
    tabledata, predicted_entities_df = setup_entity_table(entities_fullname)
    sel_start, sel_end = 0, len(predicted_entities_df)
    logger.info(f"Viewing entities {entities_fullname} from {sel_start} to {sel_end}")

    src = DataModel.g.dataset_uri(ntpath.basename(gold_objects_id), group="objects")
    logger.debug(f"Getting objects {src}")
    with DatasetManager(src, out=None, dtype="float32", fillvalue=0) as DM:
        ds_objects = DM.sources[0]
    entities_fullname = ds_objects.get_metadata("fullname")
    tabledata, gold_entities_df = setup_entity_table(entities_fullname)
    sel_start, sel_end = 0, len(gold_entities_df)
    logger.info(f"Viewing entities {entities_fullname} from {sel_start} to {sel_end}")

    from survos2.entity.instanceseg.detector_eval import analyze_detector_predictions

    analyze_detector_predictions(
        np.array(gold_entities_df), np.array(predicted_entities_df)
    )
    return [0]
...
def detector_predict(
    ...
    from entityseg.training.detector import (
        prepare_filtered_patch_dataset,
        prepare_patch_dataloaders_and_entities,
        prepare_patch_dataset,
        prepare_patch_dataloaders,
    )
    from entityseg.models.head_cnn import make_classifications
    from entityseg.training.utils import pad_vol
    from entityseg.training.detector import setup_training

    padding = (64, 64, 64)

    logger.debug(f"Detector prediction using {feature_ids} and model {model_fullname}")
    ...
    padded_vol = pad_vol(features[0], np.array(padding) // 2)
    all_patches, _ = prepare_filtered_patch_dataset(
        selected_entities, [padded_vol], flip_xy=False, bvol_dim=(64, 64, 64)
    )
    dataloader = {
        "all": DataLoader(all_patches, batch_size=1, shuffle=False, num_workers=0),
    }

    detmod, optimizer = setup_training(model_fullname, "C:\experiments")
    device = 0
    predictions, acc, logits = make_classifications(detmod, dataloader["all"], device)
    print(f"Made predictions of shape: {predictions.shape}")

    from survos2.entity.instanceseg.detector import process_predictions

    predicted_target_entities, predicted_bg_entities = process_predictions(
        selected_entities, predictions, logits, dataloader
    )

    logger.debug("Load analyzer result as objects")
    from survos2.entity.entities import load_entities_via_file

    load_entities_via_file(predicted_target_entities, flipxy=False)
    load_entities_via_file(predicted_bg_entities, flipxy=False)
    print(predicted_target_entities)
    print(predicted_bg_entities)
