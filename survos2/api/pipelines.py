...
@hug.get()
def superregion_segment(workspace: String, anno_id: String, region_id: String, feature_ids : DataURIList, dst: DataURI):
    logger.debug(f"superregion_segment with workspace {workspace}, anno {anno_id} and superregions {region_id} and features {feature_ids}")
...
def create(workspace: String, pipeline_type: String):
    ds = ws.auto_create_dataset(workspace, pipeline_type, __pipeline_group__,
                                __pipeline_dtype__, fill=__pipeline_fill__)
...
def existing(workspace: String, full: SmartBoolean = False, filter: SmartBoolean = True):
    ...
    if full:
        datasets = {'{}/{}'.format(__pipeline_group__, k): dataset_repr(v)
