...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups=8, weight_standardize=False, frame_kernel_size=1):
        super().__init__()
...
class ConvNextBlock(nn.Module):
    def __init__(
        self, dim, dim_out, *, mult=2, frame_kernel_size=1, nested_unet_depth=0, nested_unet_dim=32
    ):
        super().__init__()
        kernel_conv_kwargs = partial(kernel_and_same_pad, frame_kernel_size)
...
class FeatureMapConsolidator(nn.Module):
    def __init__(
        self, dim, *, dim_ins=tuple(), dim_outs=tuple(), resize_fmap_before=True, conv_block_fn=None
    ):
        super().__init__()
...
class XUnet(nn.Module):
    def __init__(
        ...
        for ind, ((dim_in, dim_out), nested_unet_depth, num_blocks, self_attn_blocks) in enumerate(
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        ...
                        nn.ModuleList(
                            [
                                TransformerBlock(dim_in, depth=self_attn_blocks, **attn_kwargs)
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        ...
                        nn.ModuleList(
                            [
                                TransformerBlock(dim_out, depth=self_attn_blocks, **attn_kwargs)
                                for _ in range(self_attn_blocks)
                            ]
                        ),
    ...
    def __init__(
        ...
        *,
        ...
        for ind in range(depth):
            ...
            up = nn.Sequential(
                PixelShuffleUpsample(2 * M, dim_in), nn.GroupNorm(groups, dim_in), nn.SiLU()
            )
...
def prepare_xunet(existing_model_fname=None, num_epochs=2, initial_lr=0.001, device=0):
    ...
    model3d = XUnet(
        ...
        consolidate_upsample_fmaps=True,  # whether to consolidate outputs from all upsample blocks
