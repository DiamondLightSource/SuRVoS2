...
#
# FPN code modified from medicaldetectiontoolkit
    ...
    def __init__(self, cf, conv, relu_enc="relu", relu_dec=None, operate_stride1=False):
        ...
        if operate_stride1:
            self.C0 = nn.Sequential(
                conv(len(cf.channels), sf, ks=3, pad=1, norm=cf.norm, relu=relu_enc),
            ...
            self.C1 = conv(
                ...
                sf,
                ks=7,
                stride=(2, 2, 1) if conv.dim == 3 else 2,
                pad=3,
                norm=cf.norm,
                relu=relu_enc,
            )
