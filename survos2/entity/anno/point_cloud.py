...
def chip_cluster(orig_pts, chip, offset_x, offset_y, MIN_CLUSTER_SIZE=5, 
                 method='hdbscan', plot_all= False, debug_verbose=False):
    ...
    if plot_all:
        ...
        plt.scatter(orig_pts[:,1] -offset_x , orig_pts[:,2] - offset_y, s=60, 
                    linewidth=0, c=orig_pts[:,3], alpha=1.0)
        plt.title("Before")
    ...
    if method == 'hdbscan':
        ...
        clusterer = DBSCAN(eps=20, min_samples=1).fit(X_rescaled)
    ...
    if False:    
            ...
            cluster_probs_cl = [1.0] * len(label_code_cl)
            cleaned_colors = [sns.desaturate(pal[col], sat) for col, sat 
                    in zip(label_code_cl, cluster_probs_cl)]

            plt.imshow(img_sample, cmap='gray')
            plt.title("After")
            plt.scatter(cluster_centroids[:,1] -offset_x , cluster_centroids[:,2] - offset_y, s=60, 
                        linewidth=0, c=cleaned_colors, alpha=1.0)
...
# add more clustering
def chip_cluster2(orig_pts, chip, offset_x, offset_y, MIN_CLUSTER_SIZE=5, 
                 method='hdbscan', plot_all= False, debug_verbose=False):
    X  = orig_pts.copy()
    img_sample = chip[0,:]
    subsample=False
    ...
    if method=='hdbscan':
        ...
        if plot_all:    
            ...
            plt.imshow(img_sample, cmap='gray')
            plt.title("After")
            plt.scatter(X_rescaled_cl[:,1] -offset_x , X_rescaled_cl[:,2] - offset_y, s=60, 
                        linewidth=0, c=cleaned_colors, alpha=1.0)
