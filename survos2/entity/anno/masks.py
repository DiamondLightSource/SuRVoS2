...
def create_rect_mask(mask_dim : Tuple[int,int,int]=(100,100,100),  
                    center : Union[None, Tuple[float, float, float]]= None,
                    box_dim : Tuple[int,int,int]=(1,1,1)) -> np.ndarray:
    d,w,h = mask_dim
    ...
    mask[center[0]-box_dim[0]//2:center[0]+box_dim[0]//2,
...
def paint_ellipsoids():
    ...
    total_mask = 0.1 * np.random.random((img_in.shape[0],img_in.shape[1],img_in.shape[2]))# wide_patch.shape[0]))

    for c,ecc in cluster_coords_offset[0:16]:
        
        sphere_mask  = create_ellipsoidal_mask(img_in.shape[0],img_in.shape[1],img_in.shape[2], 
                                            center=(c[0],c[1],c[2]),
                                            a=ecc[0], b=ecc[1], c=ecc[2],
...
#@jit(nopython=False)
def calc_sphere(image_shape: Tuple[int, int, int] = (100,100,100),
                center : Tuple[int, int, int]  = (40,40,40), radius : int = 30):
