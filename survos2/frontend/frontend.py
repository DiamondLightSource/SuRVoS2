...
from survos2.frontend.panels_magicgui import roi_gui, pipeline_gui, workspace_gui, update_annotation_gui
...
def frontend(cData):
    ...
    with napari.gui_qt():
        ...
        def process_pipeline(pipeline):
            ...
            with ProgressDialog(f"Processing pipeline {cData.cfg['pipeline_option']}", minVal, maxVal) as dlg:                    
        ...
        def processEvents(msg):
            if msg['data'] == 'view_pipeline':
                ...
                annotation_layer = [v for v in viewer.layers if v.name==cfg.current_annotation]
                if len(annotation_layer)==1:
                    logger.debug(f"Updating annotation in workspace {cfg.current_annotation} with label image {annotation_layer}")
        ...
        update_annotation_dockwidget = viewer.window.add_dock_widget(update_annotation_gui_widget, area='left')
        viewer.layers.events.changed.connect(lambda x: update_annotation_gui_widget.refresh_choices())
        ...
        def median(viewer, median_size=2):
            ...
            img = viewer.layers.selected[0].data.copy()
            img = img_as_ubyte(ndimage.median_filter(simple_norm(img), size=median_size))
