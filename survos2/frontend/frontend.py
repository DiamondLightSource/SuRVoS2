...
from survos2.frontend.view_fn import view_feature, view_regions, remove_layer, view_objects, view_pipeline
import warnings
...
def frontend(viewer):
            ...
            if msg["level_id"] is not None:
                ...
                def undo(v):
                    ...
                    if cfg.num_undo == 0:
                        ...
                        result = Launcher.g.run(
                            "annotations", "annotate_undo", **params
                        )
    ...
    def transfer_layer(msg):
        with progress(total=3) as pbar:
            ...
            if isinstance(selected_layer, Labels):
                ...
                load_entities_as_objects(entities_arr)
                ...
                if result:
                    ...
                    with DatasetManager(
                        dst, out=dst, dtype="float32", fillvalue=0
                    ) as DM:
                        DM.out[:] = selected_layer.data
            else:
    ...
    def processEvents(msg):
        ...
        if msg["data"] == "jump_to_slice":
            ...
            if cfg.retrieval_mode != "slice":
                ...
                view_feature(viewer, {"feature_id": "001_raw"})
    ...
    dw.Config = Config
    # from survos2.frontend.test_progress_server import progress_server
    # progress_worker = progress_server()
