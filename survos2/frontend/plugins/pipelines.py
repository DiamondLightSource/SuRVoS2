...
class PipelinesPlugin(Plugin):
    ...
    def _populate_pipelines(self):
        ...
        if not result:
            ...
            result = {}
            result[0] = params
            self.pipeline_params['superregion_segment'] = {'sr_params': {  'type':'sr2', }}
        else:
            all_categories = sorted(set(p['category'] for p in result))
            
            for i, category in enumerate(all_categories):
                self.pipeline_combo.addItem(category)
                self.pipeline_combo.model().item(i + len(self.pipeline_params) + 1).setEnabled(False)
...
class PipelineCard(Card):
    ...
    def _add_annotations_source(self):
        ...
        widget = HWidgets('Annotation:', self.annotations_source, Spacing(35), stretch=1)
    ...
    def compute_pipeline(self):
        ...
        all_params['feature_ids'] = feature_names_list #['002_gaussian_blur', '002_gaussian_blur']
