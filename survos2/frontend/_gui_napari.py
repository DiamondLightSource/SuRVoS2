...
class NapariWidget(NapariViewer):
    ...
    def dilate(self, viewer):
        ...
        img_proc = ndimage.morphology.binary_dilation(
            img_as_ubyte(img_to_ndimage), str_3D
        )
    ...
    def median(self, viewer, median_size=2):
        ...
        img_proc = img_as_ubyte(
            ndimage.median_filter(simple_norm(self.img), size=median_size)
        )
